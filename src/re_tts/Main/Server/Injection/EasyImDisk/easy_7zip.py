...
if not fl.file_exists(_7zip_path):
...
if not fl.file_exists(LOG_DIR):
    fl.create_dir(LOG_DIR)

logger = log_lib.LogStream("easy_7zip",LOG_DIR, log_lib.DEBUG, f_display=True).logger
...
def _7zip_install(_7zip_path: Path,ver="25.01"):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Failed to delete {_7zip_zip_path}: {traceback.format_exc()}")
    return _7zip_path

...
def create_empty_zip_temp_path():
    ...
    # 使用 delete=False 以便路径可以被外部库读取
    tmp = tempfile.NamedTemporaryFile(suffix=".zip", delete=False)
    tmp.close()  # 先关闭句柄，让 zipfile 可以独占打开
...
if DEBUG_WEBUI_PY:
    formatter = logging.Formatter(
        f'[%(processName)s:%(process)d][%(threadName)s:%(thread)d][%(name)s][Func:%(funcName)s] : %(message)s',datefmt='%H:%M:%S'
    )
...
class RVC_Service(rpyc.Service):
    def exposed_get_vc(self,
        ...
        ret = vc.get_vc(sid0,
                        protect,
                        protect)
        return ret[0],ret[1],ret[3] # type: ignore
